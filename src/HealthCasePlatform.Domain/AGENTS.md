...
- `OutboxMessage.Create(string type, string payload)` is **`internal static OutboxMessage`** (NOT `public static ErrorOr<T>`) — the documented exception to the "Entity = `public static ErrorOr<T> Create(...)` = only public way in" rule. The outbox is an infra relay, not a domain invariant, so there is no validation ceremony. `<InternalsVisibleTo("HealthCasePlatform.Infrastructure")>` + `<InternalsVisibleTo("HealthCasePlatform.Domain.Tests")>` exist on `Domain.csproj` for the one `SqlCaseEventOutbox` caller + the test. Its mutable processing-state setters (`ProcessedAtUtc`/`Attempts`/`LastError`) are `internal set` (not `private set`) so the Infra `OutboxDispatcher` can update them; `Type`/`Payload`/`OccurredAtUtc` stay immutable `private set`.
