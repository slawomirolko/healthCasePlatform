...
public sealed class ApproveCaseCommandHandlerTests : CaseHandlerTestBase
{
    private static readonly Guid CaseId = Guid.CreateVersion7();

    private static ApproveCaseCommand Command() => new(CaseId);
    ...
    [Fact]
    public async Task Handle_WhenCaseIsPendingDecision_ReturnsApprovedCase()
    {
        ...
        result.Value.Status.ShouldBe(CaseStatus.Approved);
