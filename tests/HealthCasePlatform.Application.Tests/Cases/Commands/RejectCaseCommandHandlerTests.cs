...
public sealed class RejectCaseCommandHandlerTests : CaseHandlerTestBase
{
    private static readonly Guid CaseId = Guid.CreateVersion7();

    private static RejectCaseCommand Command() => new(CaseId);
    ...
    public async Task Handle_WhenCaseNotFound_ReturnsNotFound()
    {
        ...
        var handler = new RejectCaseCommandHandler(repo, CreateAuditWriter());
    ...
    [Fact]
    public async Task Handle_WhenCaseIsPendingDecision_ReturnsRejectedCase()
    {
        ...
        result.Value.Status.ShouldBe(CaseStatus.Rejected);
