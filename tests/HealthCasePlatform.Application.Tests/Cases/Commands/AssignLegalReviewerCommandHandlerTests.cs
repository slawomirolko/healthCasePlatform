...
public sealed class AssignLegalReviewerCommandHandlerTests : CaseHandlerTestBase
{
    private static readonly Guid CaseId = Guid.CreateVersion7();

    private static AssignLegalReviewerCommand Command() => new(CaseId, "legal-1");
    ...
    public async Task Handle_WhenCaseNotFound_ReturnsNotFound()
    {
        ...
        var handler = new AssignLegalReviewerCommandHandler(repo, CreateAuditWriter());
    ...
    public async Task Handle_WhenReviewerIdValid_AssignsAndSaves()
    {
        ...
        result.Value.AssignedLegalReviewerId.ShouldBe("legal-1");
    ...
    public async Task Handle_WhenReviewerIdEmpty_DoesNotPersist()
    {
        ...
        var result = await handler.Handle(new AssignLegalReviewerCommand(CaseId, ""), CancellationToken.None);
